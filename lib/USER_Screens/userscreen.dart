...
import 'package:admissionguidence/models/TaskSubmitModel.dart';
...
class _UserScreenState extends State<UserScreen> {
  ...
  Future<TaskSubmitModel?> tasksubmit() async {
    var dio = Dio();

    var headers = {
      'accept': 'application/json',
      'Content-Type': 'application/x-www-form-urlencoded',
      'Cookie': 'PHPSESSID=aavs424mogrpgdf7kbmn09bp1u'
    };

    try {
      var response = await dio.post('https://admissionguidanceindia.com/appdata/task.php',
        data: { 'taskSubmit': '1',
          'user_id': '5',
          'task_id': '1'},
        options: Options(headers: headers),
      ...
      if (response.statusCode == 200) {
        var responseData = response.data is String
            ? json.decode(response.data)
            : response.data;
        return TaskSubmitModel.fromJson(responseData);
      } else {
        print('Failed with status code: ${response.statusCode}');
      }
    } catch (e) {
